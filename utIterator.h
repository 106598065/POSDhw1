...
TEST(iterator, ComboLSDFSiterator){
  Number one(1);
  Variable X("X");
  Variable Y("Y");
  Number two(2);
  ...
  //EXPECT_FALSE(itStruct->isDone());
  EXPECT_FALSE(itList->isDone());
  EXPECT_EQ("s(1, t(X, 2), Y)", itList->currentItem()->symbol());
  itList->next();
  EXPECT_EQ("1", itList->currentItem()->symbol());
  itList->next();
  EXPECT_EQ("t(X, 2)", itList->currentItem()->symbol());
...
TEST(iterator, ComboSLDFSiterator){
  Number one(1);
  Variable X("X");
  Variable Y("Y");
  Number two(2);
  ...
  //EXPECT_FALSE(itList->isDone());
  itStruct->first();
  EXPECT_EQ("[1, t(X, 2), Y]", itStruct->currentItem()->symbol());
  itStruct->next();
  EXPECT_EQ("1", itStruct->currentItem()->symbol());
  itStruct->next();
  EXPECT_EQ("t(X, 2)", itStruct->currentItem()->symbol());
...
TEST(iterator, ComboSLBFSiterator){
  Number one(1);
  Variable X("X");
  Variable Y("Y");
  Number two(2);
  ...
  Iterator<Term *> *itList = l.createBFSIterator();
  Iterator<Term *> *itStruct = x.createBFSIterator();
  //EXPECT_FALSE(itList->isDone());
  itStruct->first();
  EXPECT_EQ("[1, t(X, 2), Y]", itStruct->currentItem()->symbol());
  itStruct->next();
...
TEST(iterator, ComboLSBFSiterator){
  Number one(1);
  Variable X("X");
  Variable Y("Y");
  Number two(2);
  ...
  EXPECT_EQ("2", itList->currentItem()->symbol());
  //itList->next();
  //EXPECT_EQ("", itList->currentItem()->symbol());

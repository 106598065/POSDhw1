...
/*{
  double distance=sqrt(pow((vertex_1.x - vertex_2.x),2)+pow((vertex_1.y - vertex_2.y),2));
  return distance;
}*/
...
public:
    ...
    Triangle(vertex vertex_A, vertex vertex_B, vertex vertex_C, std::string name = "t"):
        Shape(name), v1(vertex_A), v2(vertex_B), v3(vertex_C) {
            ...
            if(isTriangle(vertex_A, vertex_B, vertex_C) == false)cout<<"It's not a triangle.";
    ...
    static bool isTriangle(vertex vertex_A, vertex vertex_B, vertex vertex_C){
        ...
        //cout << "distance b = "<< b <<endl;
        //cout << "distance c = "<< c <<endl;
